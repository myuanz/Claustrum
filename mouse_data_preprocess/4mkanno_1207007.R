...
dataDir=paste0('/home/liuyx/liuyuxuan/spa_seurat/cla/mouse/batch_normalize_renew_240115/dif_res_anno/')
...
mt_gene=rownames(Excit)[grep('^mt-.*',rownames(Excit))]
...
mt_gene=rownames(Inhibit)[grep('^mt-.*',rownames(Inhibit))]
new_assay=assay[setdiff(rownames(assay),mt_gene),]
...
mt_gene=rownames(Astrocytes)[grep('^mt-.*',rownames(Astrocytes))]
new_assay=assay[setdiff(rownames(assay),mt_gene),]
...
mt_gene=rownames(Oligo)[grep('^mt-.*',rownames(Oligo))]
new_assay=assay[setdiff(rownames(assay),mt_gene),]
...
mt_gene=rownames(Microglia)[grep('^mt-.*',rownames(Microglia))]
new_assay=assay[setdiff(rownames(assay),mt_gene),]
...
meta=OPC@meta.data
mt_gene=rownames(OPC)[grep('^mt-.*',rownames(OPC))]
new_assay=assay[setdiff(rownames(assay),mt_gene),]
...
mt_gene=rownames(Endotheial)[grep('^mt-.*',rownames(Endotheial))]
new_assay=assay[setdiff(rownames(assay),mt_gene),]
...
#剔除线粒体基因
assay=MSN@assays$RNA@counts
meta=MSN@meta.data
mt_gene=rownames(MSN)[grep('^mt-.*',rownames(MSN))]
new_assay=assay[setdiff(rownames(assay),mt_gene),]

MSN1=CreateSeuratObject(counts  = new_assay)
MSN1=AddMetaData(MSN1,meta)

MSN1 =  NormalizeData(MSN1, normalization.method = "LogNormalize", scale.factor = 10000)
MSN1 = FindVariableFeatures(MSN1, selection.method = "vst", nfeatures = 3000)
MSN1 = ScaleData(MSN1, features = rownames(MSN1))
MSN1 = RunHarmony(
    MSN1, group.by.vars="sample", plot_convergence=T,
    reduction = "pca", dims.use=1:30,
    theta=2,)
MSN1 = MSN1 %>%
    FindNeighbors(reduction="harmony", dims = 1:30) %>%
    FindClusters(verbose = FALSE,resolution = MSN_resolution) %>%
    RunUMAP(reduction="harmony", dims = 1:30)
...
Endotheial1@meta.data$Subclass=paste0(Endotheial1@meta.data$maxPredClass,'_',Endotheial1@meta.data$seurat_clusters)
...
sub_2=rbind(sub1,sub2,sub3,sub4,sub5,sub6,sub7,sub9)
